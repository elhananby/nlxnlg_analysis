        ...
        for ii_tetrode = 1:length(files_to_load)
            ...
            for ii_cell = cn
                ...
                if strcmpi(p.nlgnlx, 'nlg')
                    outfile_name = sprintf('nlg_%i-%s_Day%d_Exp%i_Session%i_TT%i_Cell%i.mat',...
                        p.animal, p.animal_name, p.day, p.experiment, ii_nses, ii_tetrode, ii_cell);
                else
                    outfile_name = sprintf('%i-%s_Day%d_Exp%i_Session%i_TT%i_Cell%i.mat',...
                        p.animal, p.animal_name, p.day, p.experiment, ii_nses, ii_tetrode, ii_cell);
                end
