...
%---------------------------
% Maya G.  - bug fixes 07/2015
% Based on Arseny Finkelstein and Gily Ginosar 04/02/2014
%--------------------------
...
ExtractMode = 1; % Maya - We want to extract all the records 
ExtractionModeArray = [];
...
%TTL_timestamps_Nlg = p.TTL_timestamps_Nlg{p.nlg_part}'-Nlg_event_delay_ms; %in ms
TTL_timestamps_Nlg = (p.TTL_timestamps_Nlg/1000)'-Nlg_event_delay_ms;
%% 1) Extracting the TTLs peaks in neuralynx data
% I switched from cross correlation to simple findpeak - detection of events with no offset. ; Didi 25.7.2015 
%debug
CSC_SamplePeriod_microsec = round( 10^6/SampleFrequency(1) );
...
% % %% 1) Extracting the TTLs peaks by computing crosscorrelation with a step function template - version 0
% % heaviside_template=[-ones(1,heaviside_template_window),ones(1,heaviside_template_window)]; %heaviside step_function imitating the rise time of the TTL pulse
% % 
% % %performing crosscorrelation between the data and the step_function
% % peaks_gily_dbg = cross_corr_threshold;
% % c_all = [];
% % ymax_all = [];
% % idx_max_all = [];
...
% %         % [c] = conv(Samples_without_DC(chunk_start:chunk_end), fliplr(heaviside_template)); % Maya - conv improved performance relative to xcorr
% %         
% %         c=c(chunk_window:end);
% % 		c_all = [c_all c];
% %         chunk_start = chunk_start + chunk_window; % update chunk start
% % 
% % 		if numel(TTL_onset_idx)>35
% % 			flag = 1;
% % 		end
...
% % 		ymax_all = [ymax_all ymax];
% % 		idx_max_all = [idx_max_all idx_max];
% % 		%peaks_gily_dbg = [peaks_gily_dbg ymax];
% %         %idx_max_sorted = sort(idx_max) +  chunk_window*(ii_chunk-1); % making this idx relative to the complete trace
% % 		idx_max_sorted = sort(idx_max) +  chunk_window_until_last*(ii_chunk-1);% making this idx relative to the complete trace
...
% %     
% %     c_all = [c_all 0];
% %     TTL_timestamps_Nlx = Timestamps_filledIn(TTL_onset_idx); % Maya - compared to CSC - < 0.5msec shift
...
% %     TTL_onset_idx = idx_max_sorted - cross_corr_detection_peak_shift; % Maya - fixing the direction of shift 
% %                                                                       % shifting the peaks by this empirical shift so they will be exactly aligned to the rise phase of the TTL
...
% %     
% %     % Saving the Figure
% %     filename_fig_to_save = fullfile(folder_name, '1_TTL_peaks_from_crosscorr');
...
temp_time_vector_TTL_Nlg = filtfilt(hamming(40),1,time_vector_TTL_Nlg); % gaussian smoothing to be robust to potential jitter
temp_time_vector_TTL_Nlx = filtfilt(hamming(40),1,time_vector_TTL_Nlx); % gaussian smoothing to be robust to potential jitter
time_vector_TTL_Nlg(1) = 1; 
time_vector_TTL_Nlx(1) = 1;




[c_Nlg_Nlx, lags] = xcorr(temp_time_vector_TTL_Nlg,temp_time_vector_TTL_Nlx);


%finding the delay in ms
[temp shift_Nlg_Nlx_ms ]=max(c_Nlg_Nlx);
% shift_Nlg_Nlx_ms= shift_Nlg_Nlx_ms- length(time_vector_TTL_Nlg);
shift_Nlg_Nlx_ms = shift_Nlg_Nlx_ms - length(temp_time_vector_TTL_Nlx); % Maya - this makes it possible to use a much longer NLX session relative to the NLG session
...
figure;
hold on;
plot(lags, c_Nlg_Nlx,'-k'); % cross correlation % GILY - I included lags in x axis
plot([ 0, 0],[min(c_Nlg_Nlx) max(c_Nlg_Nlx)],'-.r'); % plot([ length(time_vector_TTL_Nlg), length(time_vector_TTL_Nlg)],[min(c_Nlg_Nlx) max(c_Nlg_Nlx)],'-.r');
...
xlabel('lags (ms)');%xlabel('Time (ms)');

% Saving the Figure
filename_fig_to_save=fullfile(folder_name, '2_TTL_Nlg_Nlx_crosscorr');
%save the fig as .tiff
    print(filename_fig_to_save,['-f' num2str(gcf)],'-dtiff','-cmyk','-r300' );
% % %save the fig as .fig
saveas(gca,filename_fig_to_save,'fig');
...
% Maya - I'm removing this step, it introduces error to the computation
% whenever the first TTL is missed in NLX!
...
TTL_timestamps_Nlx=TTL_timestamps_Nlx+shift_Nlg_Nlx_ms*10^3; % nlx in uS is given the shift to match nlg
...
% Excluding pairs with temporal jitter of more than
...
% Saving the Figure
filename_fig_to_save = fullfile(folder_name, '3_Nlg_Nlx_clocks_sync_and_Residuals');
%save the fig as .tiff
print(filename_fig_to_save,['-f' num2str(gcf)],'-dtiff','-cmyk','-r300' );
% %save the fig as .fig
saveas(gca,filename_fig_to_save,'fig');
...
% Saving the Figure
filename_fig_to_save = fullfile(folder_name, '4_TTL_PSTH_aligned_to_peak_detected');
%save the fig as .tiff
print(filename_fig_to_save,['-f' num2str(gcf)],'-dtiff','-cmyk','-r300' );
% %save the fig as .fig
saveas(gca,filename_fig_to_save,'fig');
