...
parfor ii_Tetrode=(p.use_tetrodes)
    ...
    if ~isempty(dir(fullfile(dir_data_in ,sprintf('*bat*TT%d*TEMP.mat',ii_Tetrode))))
        ...
        Nlg_Flight_detect_spikes_CSC_per_TT_maya(p, ii_Tetrode)
    end
...
for ii_Tetrode= (p.use_tetrodes)
    ...
    filename_out_sleep	= [filename_out, '_Sleep_ONLY'];
    filename_out_behav	= [filename_out, '_Behav_ONLY'];
    ...
    %Arseny: TO FIX SHOULD BE CHANGED TO Mat2NlxSpike in the new Mat2Nlx formats
